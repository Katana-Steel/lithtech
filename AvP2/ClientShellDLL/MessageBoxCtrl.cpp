...
char szChangeSound[] = "Sounds\\Menu\\selectchange.wav";
char szSelectSound[] = "Sounds\\Menu\\select.wav";
...
void CMessageBoxCtrl::Render(HSURFACE hDestSurf)
{
	...
	{
		m_pClientDE->DrawSurfaceToSurface(hDestSurf, m_hBackgroundSurf, LTNULL, m_screenPos.x, m_screenPos.y);
	}
	...
	if (m_textButtonArray.GetSize() > 0)
	{
		...
		for (i=0; i < m_textButtonArray.GetSize(); i++)
		{
			...
			{
				int nState= LGCS_NORMAL;
				if ((int)i == m_nSelection)
					nState = LGCS_SELECTED;
				m_pFont->DrawMenu(m_textButtonArray[i].m_hString, hDestSurf, x, y, LTF_JUSTIFY_LEFT, nState);
			}
			else
			{
...
{
	_ASSERT(nIndex < (int)m_textButtonArray.GetSize());
	...
	{
		nTotalWidth+=m_textButtonArray[i].m_size.x;
	...
	for (i=0; i < nIndex; i++)
	{
		...
		// Add the text width
		nLeft+=m_textButtonArray[i].m_size.x;
	...
	rcBounding.right=rcBounding.left+m_textButtonArray[i].m_size.x;
	rcBounding.bottom=rcBounding.top+m_textButtonArray[i].m_size.y;
...
void CMessageBoxCtrl::AddMessageButton(HSTRING hString, uint32 dwMessageID)
{
	...
	int nWidth =  GetWidth()-(MB_EDGESPACE*2) / 4;
	button.m_size=m_pFont->GetTextExtentsFormat(button.m_hString, nWidth);
...
#ifdef __PSX2
/*************************************************************************************/
// PSX2: Keyboard and mouse handling is replaced with gameput input command handling
// NOTE that a command is actually selected when the select button is RELEASED

LTBOOL CMessageBoxCtrl::OnInterfaceCommand(int command)
{
	switch(command)
	{
		case COMMAND_ID_INTERFACE_ENTER:
		{
			if (m_nSelection != MB_NOSELECT)
			{
				g_pInterfaceMgr->PlayInterfaceSound(IS_SELECT,szSelectSound);
		...
		case COMMAND_ID_INTERFACE_LEFT:
		{
			if (m_nSelection != MB_NOSELECT)
			{
				m_nSelection--;
				...
				g_pInterfaceMgr->PlayInterfaceSound(IS_SELECT,szChangeSound);
				return LTTRUE;
		...
		case COMMAND_ID_INTERFACE_RIGHT:
		{
			if (m_nSelection != MB_NOSELECT)
			{
				m_nSelection++;
		...
		case COMMAND_ID_INTERFACE_CANCEL:
		{
			if (m_dwCancelID != 0)
			{
				g_pInterfaceMgr->PlayInterfaceSound(IS_ESCAPE);
				return (m_pCommandHandler->SendCommand(m_dwCancelID, 0, 0));
			}
		}
		break;
	}
...
LTBOOL CMessageBoxCtrl::HandleKeyDown(int key, int rep)
{
	...
	case VK_LEFT:
	{
		if (m_nSelection != MB_NOSELECT)
		{
			...
			if (m_nSelection < 0)
				m_nSelection = m_textButtonArray.GetSize() - 1;
			g_pInterfaceMgr->PlayInterfaceSound(IS_CHANGE,szChangeSound);
			return LTTRUE;
