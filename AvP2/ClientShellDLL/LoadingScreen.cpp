...
void CLoadingScreen::CreateCharFX(INT_CHAR *pChar)
{
	if (pChar)
	{
		...
		if( characterSet < 0 )
		{
			g_pLTClient->CPrint("CLoadingScreen::CreateCharFX : Could not find set %s.", pChar->szModel);
		}
...
LTBOOL CLoadingScreen::Init()
{
	...
	{
		switch (g_pProfileMgr->GetSpecies())
		{
		case Alien:
			CreateInterfaceSFX(FOLDER_ID_LOADSCREEN_ALIEN);
			break;
		default:
		case Marine:
			CreateInterfaceSFX(FOLDER_ID_LOADSCREEN_MARINE);
			break;
		case Predator:
			CreateInterfaceSFX(FOLDER_ID_LOADSCREEN_PREDATOR);
			break;
		}
	}
	else
...
//	m_PhotoPos = g_pLayoutMgr->GetLoadPhotoPos();
	m_TextPos = LTIntPt(320,320);
	m_PhotoPos = LTIntPt(400,100);
...
LTBOOL CLoadingScreen::Update()
{
	...
	HLTCOLOR hShadeColor = kBlack; //g_pLayoutMgr->GetShadeColor();
