...
LTBOOL CBaseParticleSystemFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	m_dwFlags		= 0;
	m_pTextureName	= "SpriteTextures\\SFX\\Particles\\smoke1.dtx";
...
LTBOOL CBaseParticleSystemFX::Update()
{
	...
	// See if we care about our parents..  I know I do!
    if (!CSpecialFX::Update())
		if(!m_basecs.bIgnoreParentRemoval)
			return LTFALSE;
			...
			while (pCur && pCur != pTail)
			{
				...
				if (fLifetime > 0.0f && fTotalLifetime > 0.0f)
				{
					...
					if (m_basecs.bAdjustParticleAlpha)
					{
						...
						{
							pCur->m_Color = m_vColor1 + (vColorRange * (1.0f - pCur->m_Alpha));
						}
					}
				}
...
int CBaseParticleSystemFX::GetNumParticles(int nDefaultNum)
{
	...
	{
		HLOCALOBJ hCamera = g_pGameClientShell->GetPlayerCameraObject();
		if (hCamera)
		{
