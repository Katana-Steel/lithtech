...
#ifdef __PSX2
// ABM 11/13/00 Moved these out of LTGUIMessagebox.h, since that file is no longer used on the PSX2
...
#else // __PSX2
	void		SetCancelCommand(uint32 CancelID) { m_dwCancelID = CancelID; } // optional "cancel" handler
#endif // __PSX2
...
#ifdef __PSX2
	uint32						m_dwCancelID;		// Command ID to send handler on a CANCEL event
#endif

#ifndef __PSX2
	CMoArray<LTGUIKeyMessage>	m_keyBindings;		// Array of key bindings
#endif // !__PSX2
	CMoArray<TextButton>		m_textButtonArray;	// Array of "text buttons" for the message box
