...
namespace
{
	HLTCOLOR   g_hTransColor = SETRGB_T(0,0,0);
...
LTBOOL CCredit::Init(char* sBuf, LTBOOL bMenu)
{
	// Sanity checks...
...
void CCredit::CreateSurface()
{
	...
	if (m_bBig)
		pFont = (nWidth <= 640) ? g_pInterfaceResMgr->GetSmallFixedFont() : g_pInterfaceResMgr->GetLargeFixedFont();
	else
		pFont = (nWidth <= 640) ? g_pInterfaceResMgr->GetTinyFixedFont() : g_pInterfaceResMgr->GetSmallFixedFont();
	...
	{
		pFont->Draw(m_aStrings[i], m_hSurface, x, y, nJustify);
		y += pFont->GetHeight();
	}
...
void CCredit::Draw()
{
	...
	if (m_bMenu)
	{
		x = m_Pos.x + g_pInterfaceResMgr->GetXOffset();
		y = m_Pos.y + g_pInterfaceResMgr->GetYOffset();
	}
	else
	{
		x = (int)(g_pInterfaceResMgr->GetXRatio() * (LTFLOAT)m_Pos.x);
		y = (int)(g_pInterfaceResMgr->GetYRatio() * (LTFLOAT)m_Pos.y);
	}
...
LTBOOL CCredits::Init(int nMode, LTBOOL bClearScreen, LTBOOL bMenu)
{
	// Sanity checks...
...
void CCredits::AddCredits()
{
	...
	while (*sBuf)
	{
		...
		if (*sBuf == '#' && *((char*)_mbsinc((const unsigned char*)sBuf)) == '#')
		{
			...
			{
				//Force looping in the menu
				m_bLoop = m_bMenu;
				return;
			}
			else if (_mbsnbcmp((const unsigned char*)sCredit, (const unsigned char*)">LOOP", 5) == 0)	// end?
			{
				m_bLoop = LTTRUE;
				return;
			}
...
void CCredits::AddCredit(char* sText)
{
	...
	if (!pCredit->Init(sText,m_bMenu)) return;
