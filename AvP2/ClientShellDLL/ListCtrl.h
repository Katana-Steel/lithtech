...
public:
	...
	int			AddControl ( CLTGUICtrl *pControl ) { 	return InsertControl( m_controlArray.GetSize(), pControl ); }
	int			InsertControl( int nIndex, CLTGUICtrl * pControl);
	...
	LTBOOL		NextSelection();
	LTBOOL		PreviousSelection();
	...
	void		UseHighlight(LTBOOL bHighlight)		{m_bHighlight = bHighlight;}
	void		SetHighlightColor(HLTCOLOR hColor)	{m_hHighlightColor = hColor;}
	void		SetIndent(LTIntPt nIndent)			{m_nIndent = nIndent; CalculateWidth();}
