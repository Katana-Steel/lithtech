...
LTBOOL CGroupCtrl::GetControlUnderPoint(int xPos, int yPos, int *pnIndex)
{
	...
	int nSize = (int)m_controlArray.GetSize();
	if (nSize == 0) return LTFALSE;
...
{
	_ASSERT(pControl);
	CGroupSubCtrl *pNewCtrl = new CGroupSubCtrl(pControl,offset,bSelectable);
...
LTBOOL CGroupCtrl::OnMouseMove(int x, int y)
{
	...
	if (bOnCtrl && m_controlArray[nControlIndex]->m_pCtrl->IsEnabled())
	{
