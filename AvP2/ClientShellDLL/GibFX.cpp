...
LTBOOL CGibFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	m_nCharacterSet		= pGib->nCharacterSet;
...
// #define FADING_GIBS
	...
	for (int i=0; i < m_nNumGibs; i++)
	{
		if (m_ActiveEmitters[i])
		{
			...
			if (bBounced = UpdateEmitter(&m_Emitters[i], i))
			{
				HandleBounce(i);
			}
...
LTBOOL CGibFX::UpdateEmitter(MovingObject* pObject, int i)
{
    ...
    if (UpdateMovingObject(LTNULL, pObject, &vNewPos))
	{
		if(m_BounceCount[i] > 32)
	        ...
	        bRet = BounceMovingObject(LTNULL, pObject, &vNewPos, &m_info, INTERSECT_HPOLY);
...
void CGibFX::HandleBounce(int nIndex)
{
	...
	if (m_bBloodSplats && !GetConsoleInt("LowViolence",0))
	{
		// Don't add blood splats on the sky...

		SurfaceType eType = GetSurfaceType(m_info);
        ...
        LTVector vTemp;
		VEC_MULSCALAR(vTemp, m_info.m_Plane.m_Normal, GetRandom(1.0f, 1.9f));
		...
		sc.Filename = pBloodFiles[GetRandom(0,3)];
...
void CGibFX::CreateMiniBloodExplosion(int nIndex)
{
	...
	char* szBlood[2] = { "SpriteTextures\\SFX\\Particles\\Blood_1.dtx",
					     "SpriteTextures\\SFX\\Particles\\Blood_2.dtx" };
...
const SFX_GIBFactory SFX_GIBFactory::registerThis;

CSpecialFX* SFX_GIBFactory::MakeShape() const
{
	return new CGibFX();
}
