...
char CSliderCtrl::m_sLeftArrow[64] = "interface\\menus\\slider_left.pcx";
char CSliderCtrl::m_sLeftArrowH[64] = "interface\\menus\\slider_left_h.pcx";
char CSliderCtrl::m_sLeftArrowD[64] = "interface\\menus\\slider_left_d.pcx";
char CSliderCtrl::m_sFullBar[64] = "interface\\menus\\slider_full.pcx";
char CSliderCtrl::m_sEmptyBar[64] = "interface\\menus\\slider_empty.pcx";
char CSliderCtrl::m_sDisBar[64] = "interface\\menus\\slider_disabled.pcx";
char CSliderCtrl::m_sRightArrow[64] = "interface\\menus\\slider_right.pcx";
char CSliderCtrl::m_sRightArrowH[64] = "interface\\menus\\slider_right_h.pcx";
char CSliderCtrl::m_sRightArrowD[64] = "interface\\menus\\slider_right_d.pcx";
                              ...
                              int nSliderWidth, LTBOOL bNumDisplay, int *pnValue)
{
	...
	m_pNumFont	= pFont;
	if (m_pNumFont)
		m_nNumHeight = m_pNumFont->GetHeight();
	...
	int barOffset = m_dwHeight / 3;

	//set basic bar rect
	m_rcBar.left = m_rcLeft.right + 8;
	m_rcBar.right = m_rcRight.left - 8;
	m_rcBar.top = barOffset;
	m_rcBar.bottom =  m_dwHeight - barOffset;
...
void CSliderCtrl::SetSliderPos(int nPos)
{
	...
	// Send the command
	if ( !m_bInUpdate && m_pCommandHandler && m_nSliderPos != nOldPos)
	{
		m_pCommandHandler->SendCommand(m_dwCommandID, m_dwParam1, m_dwParam2);
	}
}


//this function sets up a notification when the control's value changes
void CSliderCtrl::NotifyOnChange(uint32 dwCommandID, CLTGUICommandHandler *pCommandHandler, uint32 dwParam1, uint32 dwParam2)
{
...
LTBOOL CSliderCtrl::OnLeft()
{
	...
	{
		SetSliderPos(nPos);
		g_pInterfaceMgr->PlayInterfaceSound(IS_LEFT);
        return LTTRUE;
	}

	g_pInterfaceMgr->PlayInterfaceSound(IS_NO_SELECT);
    return LTFALSE;
}
...
LTBOOL CSliderCtrl::OnRight()
{
	...
	{
		SetSliderPos(nPos);
		g_pInterfaceMgr->PlayInterfaceSound(IS_RIGHT);
        return LTTRUE;
	}
...
void CSliderCtrl::Render ( HSURFACE hDestSurf )
{
	...
	int xPos = m_pos.x;
	int yPos = m_pos.y + 2;
	if (m_pFont->IsMenuFriendly())
		m_pFont->DrawMenu(m_hText, hDestSurf, xPos, yPos, LTF_JUSTIFY_LEFT, GetState());
	else
	...
	if (m_bNumDisplay)
	{
		...
		yPos = m_pos.y + ((int)m_dwHeight - m_nNumHeight)/2;;
		m_pNumFont->Draw(szVal, hDestSurf, xPos, yPos, LTF_JUSTIFY_CENTER, color);
	}


}

void CSliderCtrl::SetNumericDisplay(LTBOOL bDisplay, LTBOOL bTextOnly, CLTGUIFont *pFont) 
{
