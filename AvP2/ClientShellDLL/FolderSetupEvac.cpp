...
CFolderSetupEvac::CFolderSetupEvac()
{
...
LTBOOL CFolderSetupEvac::Build()
{
	...
	CreateTitle(IDS_TITLE_SETUP_EVAC);
	UseLogo(LTFALSE);
	...
	rect = g_pLayoutMgr->GetFolderCustomRect(FOLDER_ID_HOST_SETUP_EVAC, "DescriptionRect");
	...
	hTemp = g_pLTClient->FormatString(IDS_GAME_DESCRIPTION_EVAC);
	...
	pSlider->SetSliderRange(1, 50);
	pSlider->SetSliderIncrement(1);
	pSlider->SetNumericDisplay(LTTRUE, LTTRUE);

	// Time limit
	pSlider = AddSlider(IDS_EVAC_TIME, IDS_HELP_EVAC_TIME, kWidth, kSlider, &m_nTimeLimit);
	...
	// Defender race
	pCycle = AddCycleItem(IDS_EVAC_RACE, IDS_HELP_EVAC_RACE, kWidth - 25, 25, &m_nDefenderRace);
	...
	// Defender lives
	pSlider = AddSlider(IDS_EVAC_LIVES, IDS_HELP_EVAC_LIVES, kWidth, kSlider, &m_nDefenderLives);
...
uint32 CFolderSetupEvac::OnCommand(uint32 dwCommand, uint32 dwParam1, uint32 dwParam2)
{
	switch(dwCommand)
	{
		case CMD_DEFENDERS:
...
void CFolderSetupEvac::OnFocus(LTBOOL bFocus)
{
	FocusSetup(bFocus);
...
void CFolderSetupEvac::FocusSetup(LTBOOL bFocus)
{
	CServerOptions *pOptions = g_pServerOptionMgr->GetCurrentCfg();

	if(bFocus)
	{
		m_nMaxPlayers = (int)pOptions->nEvac_MaxTotalPlayers;
		m_nRoundLimit = (int)pOptions->nEvac_RoundLimit;
		m_nTimeLimit = (int)pOptions->nEvac_TimeLimit;

		m_nDefenderRace = (int)pOptions->nEvac_DefenderRace;
		m_nDefenderLives = (int)pOptions->nEvac_DefenderLives;
		m_nAttackerRace = (int)pOptions->nEvac_AttackerRace;
		m_nAttackerLives = (int)pOptions->nEvac_AttackerLives;
	...
	else
	{
		...
		pOptions->nEvac_MaxTotalPlayers = (uint8)m_nMaxPlayers;
		pOptions->nEvac_RoundLimit = (uint8)m_nRoundLimit;
		pOptions->nEvac_TimeLimit = (uint8)m_nTimeLimit;

		pOptions->nEvac_DefenderRace = (uint8)m_nDefenderRace;
		pOptions->nEvac_DefenderLives = (uint8)m_nDefenderLives;
		pOptions->nEvac_AttackerRace = (uint8)m_nAttackerRace;
		pOptions->nEvac_AttackerLives = (uint8)m_nAttackerLives;
