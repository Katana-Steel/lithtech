...
LTBOOL CPolygonDebrisFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	{
		g_cvarPolyDebrisMinDistScale.Init(g_pLTClient, "PolyDebrisMinDistScale", LTNULL, 0.1f);
	}

	if (!g_cvarPolyDebrisMaxDistScale.IsInitted())
	{
		g_cvarPolyDebrisMaxDistScale.Init(g_pLTClient, "PolyDebrisMaxDistScale", LTNULL, 3.0f);
	}
...
LTBOOL CPolygonDebrisFX::IsValidDebris(int i)
{
	...
	return (i < m_nNumPolies && m_bValidPolies[i]);
}
...
{
	if (i < 0 || i >= m_ds.nNumDebris || i != m_nNumPolies) return;
	...
	pls.fLifeTime			= m_fDebrisLife[i];
	pls.fAlphaLifeTime		= m_fDebrisLife[i];
	...
	if (!m_Polies[m_nNumPolies].Init(&pls) ||
		!m_Polies[m_nNumPolies].CreateObject(m_pClientDE))
	{
		return;
	}
	else
	{
        m_bValidPolies[m_nNumPolies] = LTTRUE;
	}
...
void CPolygonDebrisFX::RemoveDebris(int i)
{
	...
	// Once all the polies are invalid, we'll stop updating...

    m_bValidPolies[i] = LTFALSE;
}
...
LTBOOL CPolygonDebrisFX::Update()
{
	if (CDebrisFX::Update())
	{
		for (int i=0; i < m_nNumPolies; i++)
		{
			...
			if (!m_cs.PolyDebrisFX.bShowTrail)
			{
                LTVector vDir = m_Emitters[i].m_vLastPos - m_Emitters[i].m_vPos;
...
const SFX_PolyDebrisFactory SFX_PolyDebrisFactory::registerThis;

CSpecialFX* SFX_PolyDebrisFactory::MakeShape() const
{
	return new CPolygonDebrisFX();
}
