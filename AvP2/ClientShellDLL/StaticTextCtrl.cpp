...
void CStaticTextCtrl::Render ( HSURFACE hDestSurf )
{
	...
	// Calculate a position based off the justification...
	LTIntPt pt = m_pos;
...
//		if (m_pFont->IsChromaKeyed())
			g_pLTClient->DrawSurfaceToSurfaceTransparent(hDestSurf, m_hSurface, LTNULL, pt.x, pt.y, m_hTransColor);
/*		else
		{
...
//			m_pClientDE->GetOptimized2DColor(oldColor);
			...
			if (color == kWhite)
			{
				...
				g_pLTClient->DrawSurfaceToSurface(hDestSurf, m_hSurface, LTNULL, pt.x, pt.y);
			}
	...
	if (dwWidth != m_dwWidth || dwHeight != m_dwHeight)
	{
		...
		if (m_hSurface)
		{
			...
			if (m_pFont->IsMenuFriendly())
			{
				if (m_bFixedWidth)
				{
					m_pFont->DrawMenuFormat(m_hString, m_hSurface, 0, 0, m_dwWidth, LGCS_NORMAL);
				}
				else
				{
					m_pFont->DrawMenu(m_hString, m_hSurface, 0, 0, LTF_JUSTIFY_LEFT, LGCS_NORMAL);
				}
			}
			else
			{
