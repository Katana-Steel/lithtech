...
class CMusic
{
	public :
		...
		void ProcessMusicMessage(char const* pMessage);

		// Begin playing music
		void Play(int32 nIntensity = -1, int32 nStart = LTDMEnactInvalid );

		// Stop playing music
		void Stop(const LTDMEnactTypes nStart = LTDMEnactInvalid );

		// Pause playing music
		void Pause(const LTDMEnactTypes nStart);
		...
		void ChangeIntensity(const int nIntensity, const LTDMEnactTypes nStart);
	...
	private:
		...
		// Queued up messages which were received before we were fully initialized.
		std::string m_strQueuedMessages;
};
