...
BOOL CLTWnd::InitFromBitmap(int nControlID, char* szWndName, CLTWnd* pParentWnd, char* szBitmap, int xPos, int yPos, DWORD dwFlags, DWORD dwState)
{
	...
	// Create the surface
	HSURFACE hSurf = g_pInterface->CreateSurfaceFromBitmap(szBitmap);
	if(!hSurf)
	{
...
BOOL CLTWnd::Init(int nControlID, char* szWndName, CLTWnd* pParentWnd, HSURFACE hSurf, int xPos, int yPos, DWORD dwFlags, DWORD dwState)
{
	...
	{
		DWORD x, y;
		g_pInterface->GetSurfaceDims(hSurf, &x, &y);
		...
		if((m_xPos == COORD_CENTER) || (m_yPos == COORD_CENTER))
		{
			...
			if(m_pParentWnd)
			{
				...
				g_pInterface->GetSurfaceDims(m_pParentWnd->GetSurface(),&xParent,&yParent);
				m_xPos = (xParent-x)/2;
				m_yPos = (yParent-y)/2;
			}
			else
	...
	if((m_dwState & LTWS_CLOSED) || (dwParentState & (LTWS_CLOSED | LTWS_MINIMIZED)))
...
void CLTWnd::RemoveAllChildren()
{
	...
	while (pos)
	{
		...
		// (the TERM macro checks the pointer)
		TERM(pWnd);
	}
...
/*#ifdef _DEBUG
	...
	{
		if (hSurfTemp)
			g_pInterface->DeleteSurface(hSurfTemp);
		return NULL;
	}
	return hSurfTemp;
}
...
{
	m_hColorTransparent = g_pInterface->CreateColor(r, g, b, FALSE);
}
...
{
	g_pInterface->DeleteColor(m_hColorTransparent);
}
...
BOOL CLTWnd::ShowWindow(BOOL bShow, BOOL bPlaySound, BOOL bAnimate)
{
	...
	if(m_bVisible)
	{
		...
		if(!bShow)
		{
			...
			if(bPlaySound)
				g_pPrClientShell->PlaySoundLocal("/interface/sounds/HideWindow.wav");
		}
	}
	else
	{
...
BOOL CLTWnd::SetWindowPos(CLTWnd* pInsertAfter, int xPos, int yPos, int nWidth, int nHeight, DWORD dwFlags)
{
	...
	{
		RemoveFromParent();
		m_pParentWnd->m_lstChildren.InsertBefore(pInsertAfter->GetPos(),this);
	}
	else
	{
	...
	if (m_pParentWnd)
	{
		...
		if (rcSource.IntersectRect(rcSurf, rcClip))
		{
			...
			if (m_hSurf)
			{
				...
				{
					if(g_pInterface->DrawSurfaceToSurfaceTransparent(hSurfDest,m_hSurf, &drcSource, ptIntersect.x, ptIntersect.y, m_hColorTransparent) != LT_OK)
						return FALSE;
				}
				else
				{
					if(g_pInterface->DrawSurfaceToSurface(hSurfDest, m_hSurf, &drcSource, ptIntersect.x, ptIntersect.y) != LT_OK)
						return FALSE;
				}
			}
		}
...
CLTWnd* CLTWnd::GetWndFromPt(int x, int y)
{
	...
	// If we're here, then the mouse wasn't clicked in any of our subwindows
	//if(PtInWnd(xPos,yPos))
	//{
		// No other window could possibly be in front of us so we're done
		return this;
	//}
}
...
BOOL CLTWnd::HandleLButtonUp(int xPos, int yPos)
{
	if(s_pWndCapture)
	{
		s_pWndCapture->OnLButtonUp(xPos,yPos);
		return TRUE;
	}
...
BOOL CLTWnd::HandleLButtonDblClick(int xPos, int yPos)
{
	if(s_pWndCapture)
	{
		s_pWndCapture->OnLButtonDblClick(xPos,yPos);
		return TRUE;
	}
...
BOOL CLTWnd::HandleRButtonDown(int xPos, int yPos)
{
	if(s_pWndCapture)
	{
		s_pWndCapture->OnRButtonDown(xPos,yPos);
		return TRUE;
	}
...
BOOL CLTWnd::HandleRButtonUp(int xPos, int yPos)
{
	if(s_pWndCapture)
	{
		s_pWndCapture->OnRButtonUp(xPos,yPos);
		return TRUE;
	}
