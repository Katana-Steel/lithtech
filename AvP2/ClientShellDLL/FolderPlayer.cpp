...
LTBOOL CFolderPlayer::Build()
{
	...
	AddLink(IDS_PLAYER, FOLDER_CMD_MP_PLAYER, IDS_HELP_PLAYER)->Enable(LTFALSE);


	// Grab some custom values out of the attributes
	kNameWidth = g_pLayoutMgr->GetFolderCustomInt(FOLDER_ID_PLAYER, "NameWidth");
	kWidth = g_pLayoutMgr->GetFolderCustomInt(FOLDER_ID_PLAYER, "ColumnWidth");
	kSlider = g_pLayoutMgr->GetFolderCustomInt(FOLDER_ID_PLAYER, "SliderWidth");

	kTeamPos = g_pLayoutMgr->GetFolderCustomPoint(FOLDER_ID_PLAYER, "TeamPos");
	kClassPos = g_pLayoutMgr->GetFolderCustomPoint(FOLDER_ID_PLAYER, "ClassPos");
	kDescription = g_pLayoutMgr->GetFolderCustomRect(FOLDER_ID_PLAYER, "DescriptionRect");
	...
	pos = g_pLayoutMgr->GetFolderCustomPoint(FOLDER_ID_PLAYER,"PrevTeam");
	...
	pos = g_pLayoutMgr->GetFolderCustomPoint(FOLDER_ID_PLAYER,"NextTeam");
	AddFixedControl(pBMPCtrl, kCustomGroup0, pos, LTTRUE);
	...
	pos = g_pLayoutMgr->GetFolderCustomPoint(FOLDER_ID_PLAYER,"PrevClass");
	AddFixedControl(pBMPCtrl, kCustomGroup0, pos, LTTRUE);
	...
	pos = g_pLayoutMgr->GetFolderCustomPoint(FOLDER_ID_PLAYER,"NextClass");
	AddFixedControl(pBMPCtrl, kCustomGroup0, pos, LTTRUE);


	// Setup the model display information
	sChar.vPos = g_pLayoutMgr->GetFolderCustomVector(FOLDER_ID_PLAYER,"CharBasePos");
...
void CFolderPlayer::FocusSetup(LTBOOL bFocus)
{
...
void CFolderPlayer::BuildClassLists()
{
	for(int s = 0; s < 4; s++)
	{
		m_nCharCount[s] = 0;
...
void CFolderPlayer::UpdateCurrentSelection()
{
...
void CFolderPlayer::CreatePlayerModel()
{
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	CFolderPlayer::UpdateDescriptions
//
//	PURPOSE:	Update the character description text
//
// ----------------------------------------------------------------------- //

void CFolderPlayer::UpdateDescriptions()
{
	// A temp string
