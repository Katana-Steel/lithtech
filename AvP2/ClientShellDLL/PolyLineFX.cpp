...
LTBOOL CPolyLineFX::Init(SFXCREATESTRUCT* psfxCreateStruct)
{
	...
	{
		g_vtTexture.Init(g_pLTClient, "Tex", "Tracer_1", 0.0f);
	}
...
LTBOOL CPolyLineFX::Draw(ILTCustomDraw *pDraw)
{
	...
	if (g_vtTestingTexture.GetFloat() > 0.0f)
	{
		...
		{
			char buf[256];
			sprintf(buf, "SpriteTextures\\SFX\\Polys\\%s.dtx", g_vtTexture.GetStr());
	...
	HLOCALOBJ hCamera = g_pGameClientShell->GetCameraMgr()->GetCameraObj(g_pGameClientShell->GetPlayerCamera());
