...
CFolderSetupTeamDM::CFolderSetupTeamDM()
{
...
LTBOOL CFolderSetupTeamDM::Build()
{
	...
	CreateTitle(IDS_TITLE_SETUP_TEAMDM);
	UseLogo(LTFALSE);
	...
	kWidth = g_pLayoutMgr->GetFolderCustomInt(FOLDER_ID_HOST_SETUP_TEAMDM, "ColumnWidth");
	kSlider = g_pLayoutMgr->GetFolderCustomInt(FOLDER_ID_HOST_SETUP_TEAMDM, "SliderWidth");
	...
	rect = g_pLayoutMgr->GetFolderCustomRect(FOLDER_ID_HOST_SETUP_TEAMDM, "DescriptionRect");
	...
	hTemp = g_pLTClient->FormatString(IDS_GAME_DESCRIPTION_TEAMDM);
...
uint32 CFolderSetupTeamDM::OnCommand(uint32 dwCommand, uint32 dwParam1, uint32 dwParam2)
{
	switch(dwCommand)
	{
		case CMD_MAXPLAYERS:
...
void CFolderSetupTeamDM::OnFocus(LTBOOL bFocus)
{
	FocusSetup(bFocus);
...
void CFolderSetupTeamDM::FocusSetup(LTBOOL bFocus)
{
	CServerOptions *pOptions = g_pServerOptionMgr->GetCurrentCfg();

	if(bFocus)
	{
		m_nMaxPlayers = (int)pOptions->nTeam_MaxTotalPlayers;

		for(int i = 0; i < MAX_TEAMS; i++)
			m_nMaxTeams[i] = (int)pOptions->nTeam_MaxPlayers[i];

		m_nFragLimit = (int)pOptions->nTeam_FragLimit;
		m_nScoreLimit = (int)pOptions->nTeam_ScoreLimit;
		m_nTimeLimit = (int)pOptions->nTeam_TimeLimit;
		m_bLifecycle = (LTBOOL)pOptions->nTeam_Lifecycle;
	...
	else
	{
		...
		pOptions->nTeam_MaxTotalPlayers = (uint8)m_nMaxPlayers;

		for(int i = 0; i < MAX_TEAMS; i++)
			pOptions->nTeam_MaxPlayers[i] = (int)m_nMaxTeams[i];

		pOptions->nTeam_FragLimit = (uint8)m_nFragLimit;
		pOptions->nTeam_ScoreLimit = (uint16)m_nScoreLimit;
		pOptions->nTeam_TimeLimit = (uint8)m_nTimeLimit;
		pOptions->nTeam_Lifecycle = (uint8)m_bLifecycle;
...
void CFolderSetupTeamDM::Escape()
{
