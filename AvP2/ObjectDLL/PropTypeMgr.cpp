...
LTBOOL CPropTypeMgr::Init(ILTCSBase *pInterface, const char* szAttributeFile)
{
	...
	{
		PROPTYPE* pPropType = new PROPTYPE;
...
LTBOOL PROPTYPE::Init(CButeMgr & buteMgr, char* aTagName)
{
	...
	str = buteMgr.GetString(aTagName, PTMGR_SURFACE);
	if (!str.IsEmpty())
	{
		strncpy(szSurface, (char*)(LPCSTR)str, ARRAY_LEN(szSurface));
	}
...
void PROPTYPE::Cache(CPropTypeMgr* pPropTypeMgr)
{
	...
	if (szSurface[0])
	{
		SURFACE* pSurface = g_pSurfaceMgr->GetSurface(szSurface);
		if (pSurface)
		{
			pSurface->Cache(g_pSurfaceMgr);
		}
	}
}
...
LTBOOL CPropTypeMgrPlugin::PopulateStringList(char* const * aszStrings, uint32* pcStrings,
