...
#define DF_TEAROBJECT			(1<<5)	// Make the door act as an alien tear object
...
class Door : public GameBase
{
	public :
		...
		typedef DoorList::const_iterator DoorIterator;

		static DoorList::const_iterator BeginDoors() { return m_sDoorList.begin(); }
		static DoorList::const_iterator EndDoors()   { return m_sDoorList.end(); }
	...
	public:
		...
		void	SetLocked(LTBOOL bLocked)		{ m_bLocked = bLocked; }
	...
	protected :
		...
		HSTRING	m_hstrTearSound;		// Sound played when tear door torn
		HSTRING	m_hstrAttachments;		// Names of attached objects

		HSTRING m_hstrOpenCmd;			// Command to send when door is open
		HSTRING m_hstrOpeningCmd;		// Command to send when door starts to open
		HSTRING m_hstrCloseCmd;			// Command to send when door is closed
		HSTRING m_hstrClosingCmd;		// Command to send when door starts to close
	...
	// should be added here (to keep them together)...
		...
		// See through, shoot through properties (for AI's).
	...
	private :
		...
		static DoorList  m_sDoorList;
};

// AlienTearDoor Class
class AlienTearDoor : public Door
{
};
