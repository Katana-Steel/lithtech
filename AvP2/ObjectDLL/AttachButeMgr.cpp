...
#define ABM_ATTACHMENT_ALPHA				"Alpha"
...
int CAttachButeMgr::GetAttachmentIDByName(const char *szName)
{
	...
	{
		if ( !_stricmp(szName, GetAttachmentName(iAttachmentID)) )
		{
...
CString CAttachButeMgr::GetAttachmentProperties(int nAttachmentID)
{
	if (nAttachmentID < 0 || nAttachmentID > m_cAttachmentID) return "";
	sprintf(s_aTagName, "%s%d", ABM_ATTACHMENT, nAttachmentID);
	return m_buteMgr.GetString(s_aTagName, ABM_ATTACHMENT_PROPERTIES, CString());
}
...
LTFLOAT CAttachButeMgr::GetAttachmentAlpha(int nAttachmentID)
{
	if (nAttachmentID < 0 || nAttachmentID > m_cAttachmentID) return 1.0f;
	sprintf(s_aTagName, "%s%d", ABM_ATTACHMENT, nAttachmentID);
	return (LTFLOAT)m_buteMgr.GetDouble(s_aTagName, ABM_ATTACHMENT_ALPHA);
}




int CAttachButeMgr::GetNumRequirements(char *szModel)
{
	int numRequired = 0;
	int n = 0;
	CString sTestModel;
...
int CAttachButeMgr::GetRequirementIDs(char *szModel, int *pBuf, int nBufLen)
{
	int numRequired = 0;
	int n = 0;
	CString sTestModel;
	...
	while (n < m_cRequirementID && numRequired < nBufLen)
	{
		...
		if (!_strnicmp(sTestModel.GetBuffer(), szModel, strlen(sTestModel.GetBuffer()) - 1))
		{
			pBuf[numRequired] = n;
			numRequired++;
		}
