...
void WorldModelDebris::Start(DVector *pvRotationPeriods, DVector* pvVel)
{
	...
	DFLOAT fMag = VEC_MAGSQR(*pvRotationPeriods);
	if (fMag > 0.001f)
	{
		m_bRotate = DTRUE;
...
void WorldModelDebris::Save(HMESSAGEWRITE hWrite, DDWORD dwSaveFlags)
{
	...
	pServerDE->WriteToMessageFloat(hWrite, m_fLastTime - g_pLTServer->GetTime());
}
...
void WorldModelDebris::Load(HMESSAGEREAD hRead, DDWORD dwLoadFlags)
{
	...
	m_fLastTime = pServerDE->ReadFromMessageFloat(hRead) + g_pLTServer->GetTime();
}
