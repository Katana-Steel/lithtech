...
void CreateDebris(CLIENTDEBRIS & cd)
{
    ...
    g_pLTServer->WriteToMessageVector(hMessage, &cd.vPos);
    g_pLTServer->WriteToMessageByte(hMessage, cd.nDebrisId);
    g_pLTServer->WriteToMessageVector(hMessage, &cd.vVelOffset);
...
void CreateMultiDebris(CLIENTDEBRIS & cd)
{
    ...
    HMESSAGEWRITE hMessage = g_pLTServer->StartInstantSpecialEffectMessage(&cd.vPos);
    g_pLTServer->WriteToMessageByte(hMessage, SFX_MULTI_DEBRIS_ID);
    g_pLTServer->WriteToMessageRotation(hMessage, &cd.rRot);
    g_pLTServer->WriteToMessageVector(hMessage, &cd.vPos);
    g_pLTServer->WriteToMessageByte(hMessage, cd.nDebrisId);
    g_pLTServer->WriteToMessageVector(hMessage, &cd.vVelOffset);
...
void CreatePropDebris(LTVector & vPos, LTVector & vDir, uint8 nDebrisId, LTVector vVelOffset)
{
	CLIENTDEBRIS cd;
...
LTRESULT CDebrisPlugin::PreHook_EditStringList(const char* szRezPath, const char* szPropName, char* const * aszStrings, uint32* pcStrings, const uint32 cMaxStrings, const uint32 cMaxStringLength)
{
	// See if we can handle the property...

	if (_stricmp(DEBRIS_PROPERTY_NAME, szPropName) == 0)
	{
