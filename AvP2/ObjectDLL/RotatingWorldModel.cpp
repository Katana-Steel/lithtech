...
void RotatingWorldModel::InitialUpdate()
{
	...
	if (m_eState != RWM_OFF)
	{
		SetNextUpdate(RWM_UPDATE_DELTA, RWM_UPDATE_DELTA + 1.0f);
		SetNormalRotation();
	}

	// Can't push this guy around.
...
void RotatingWorldModel::UpdateSpinUp()
{
	...
	m_vSpinTimeLeft.x -= g_pLTServer->GetFrameTime();
	...
	m_vSpinTimeLeft.y -= g_pLTServer->GetFrameTime();
	...
	m_vSpinTimeLeft.z -= g_pLTServer->GetFrameTime();
...
void RotatingWorldModel::Save(HMESSAGEWRITE hWrite, uint32 dwSaveFlags)
{
	...
	g_pLTServer->WriteToMessageFloat(hWrite, m_fLastTime - g_pLTServer->GetTime());
	g_pLTServer->WriteToMessageFloat(hWrite, m_fStartTime - g_pLTServer->GetTime());
	...
	g_pLTServer->WriteToMessageByte(hWrite, (uint8)m_nShadowAxis);
...
void RotatingWorldModel::Load(HMESSAGEREAD hRead, uint32 dwLoadFlags)
{
	...
	m_fLastTime			= g_pLTServer->ReadFromMessageFloat(hRead) + g_pLTServer->GetTime();
	m_fStartTime		= g_pLTServer->ReadFromMessageFloat(hRead) + g_pLTServer->GetTime();
	...
	m_nShadowAxis		= (uint32)g_pLTServer->ReadFromMessageByte(hRead);
...
// ----------------------------------------------------------------------- //
//
//	ROUTINE:	RotatingWorldModel::CapRotation
//
//	PURPOSE:	Cap the rotation values...
//
// ----------------------------------------------------------------------- //

void RotatingWorldModel::CapRotation()
{
