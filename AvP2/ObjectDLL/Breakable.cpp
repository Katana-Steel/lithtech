...
#ifdef _WIN32
	ADD_STRINGPROP_FLAG(BreakSound, "Sounds\\Event\\Break.wav", PF_FILENAME)
#else
	ADD_STRINGPROP_FLAG(BreakSound, "Sounds/Event/Break.wav", PF_FILENAME)
#endif
...
#ifdef _WIN32
	ADD_STRINGPROP_FLAG(ImpactSound, "Sounds\\Event\\BreakImpact.wav", PF_FILENAME)
#else
	ADD_STRINGPROP_FLAG(ImpactSound, "Sounds\\Event\\BreakImpact.wav", PF_FILENAME)
#endif
...
uint32 Breakable::EngineMessageFn(uint32 messageID, void *pData, float fData)
{
	switch (messageID)
	{
		case MID_UPDATE:
		{
			...
			{
				// Hardcode the flags in case the door class did something weird to them...
				uint32 dwFlags;
				g_pLTServer->Common()->GetObjectFlags(m_hObject, OFT_Flags, dwFlags);
				g_pLTServer->Common()->SetObjectFlags(m_hObject, OFT_Flags, FLAG_VISIBLE | FLAG_TOUCH_NOTIFY);
				g_pLTServer->Common()->SetObjectFlags(m_hObject, OFT_Flags2, 0);
			}
...
void Breakable::UpdateBreaking()
{
	...
	if (m_vAdjust.x)
	{
		fDelta = ((m_vSign.x > 0.0f ? m_fRotVel : -m_fRotVel) * g_pLTServer->GetFrameTime());
	...
	if (m_vAdjust.y)
	{
		fDelta = ((m_vSign.y > 0.0f ? m_fRotVel : -m_fRotVel) * g_pLTServer->GetFrameTime());
	...
	if (m_vAdjust.z)
	{
		fDelta = ((m_vSign.z > 0.0f ? m_fRotVel : -m_fRotVel) * g_pLTServer->GetFrameTime());
...
void Breakable::TriggerMsg(HOBJECT hSender, HSTRING hMsg)
{
    char* pMsg = g_pLTServer->GetStringData(hMsg);
	if (!pMsg) return;

	if (_stricmp(pMsg, "Break") == 0)
	{
		StartBreak(hSender);
		return;
	}

	Door::TriggerMsg(hSender, hMsg);
}
