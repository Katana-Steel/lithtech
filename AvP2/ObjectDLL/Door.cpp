...
#define	DOOR_DEFAULT_DEACTIVATION_TIME	20.0f
...
static char *g_szAttachList		= "ATTACHLIST";
static char *g_szDetach			= "DETACH";
static char *g_szDetachList		= "DETACHLIST";

// Static local variables
Door::DoorList  Door::m_sDoorList;
...
BEGIN_CLASS(Door)
	...
	PROP_DEFINEGROUP(StateFlags, PF_GROUP4)
        ADD_BOOLPROP_FLAG_HELP(ActivateTrigger, LTTRUE, PF_GROUP4, "This must be set to FALSE if door is to be used as an Alien tear object.")
	    ...
	    ADD_BOOLPROP_FLAG_HELP(AlienTearObject, LTFALSE, PF_GROUP4, "Set this to true if you want the door to act as an Alien tearable object.  ActivateTrigger must be set to FALSE above.")
	...
	ADD_REALPROP(MoveDist, 124.0f)
	ADD_CHROMAKEY_FLAG(FALSE, 0)
	ADD_VECTORPROP_VAL(MoveDir, 1.0f, 0.0f, 0.0f)
	...
	ADD_STRINGPROP(OpeningCommand, "")
	ADD_STRINGPROP(ClosedCommand, "")
	ADD_STRINGPROP(ClosingCommand, "")
	...
	ADD_BOOLPROP_FLAG(AICanShootThrough, LTFALSE, 0)
	ADD_BOOLPROP_FLAG(AICanSeeThrough, LTFALSE, 0)
	ADD_STRINGPROP_FLAG_HELP(TearSound, "", PF_STATICLIST, "Not used.")
END_CLASS_DEFAULT_FLAGS_PLUGIN(Door, GameBase, NULL, NULL, 0, CDoorPlugin)

BEGIN_CLASS(AlienTearDoor)
	ADD_DESTRUCTIBLE_MODEL_AGGREGATE(PF_GROUP1, 0)

	//Override destructable model stuff
		ADD_REALPROP_FLAG(HitPoints, 1000.0f, PF_GROUP1)
		ADD_REALPROP_FLAG(MaxHitPoints, 1000.0f, PF_GROUP1)
		ADD_REALPROP_FLAG(Armor, 1.0f, PF_GROUP1)
		ADD_REALPROP_FLAG(MaxArmor, 1.0f, PF_GROUP1)
		ADD_LONGINTPROP_FLAG(DamageTriggerCounter, 0, PF_GROUP1 | PF_HIDDEN)
		ADD_STRINGPROP_FLAG(DamageTriggerTarget, "", PF_GROUP1 | PF_HIDDEN)
		ADD_STRINGPROP_FLAG(DamageTriggerMessage, "", PF_GROUP1 | PF_HIDDEN)
		ADD_LONGINTPROP_FLAG(DamageTriggerNumSends, 1, PF_GROUP1 | PF_HIDDEN)
		ADD_STRINGPROP_FLAG(DamagerMessage, "", PF_GROUP1 | PF_HIDDEN)
		ADD_STRINGPROP_FLAG(DeathTriggerTarget, "", PF_GROUP1)
		ADD_STRINGPROP_FLAG(DeathTriggerMessage, "", PF_GROUP1) 
		ADD_STRINGPROP_FLAG(PlayerDeathTriggerTarget, "", PF_GROUP1 | PF_HIDDEN)
		ADD_STRINGPROP_FLAG(PlayerDeathTriggerMessage, "", PF_GROUP1 | PF_HIDDEN)
		ADD_STRINGPROP_FLAG(KillerMessage, "", PF_GROUP1 | PF_HIDDEN)
	...
	ADD_REALPROP_FLAG(Mass, DOOR_DEFAULT_MASS, PF_GROUP1 | PF_HIDDEN)
	PROP_DEFINEGROUP(StateFlags, PF_GROUP4)
        ADD_BOOLPROP_FLAG_HELP(ActivateTrigger, LTFALSE, PF_GROUP4, "This must be set to FALSE if door is to be used as an Alien tear object.")
        ADD_BOOLPROP_FLAG(StartOpen, LTFALSE, PF_GROUP4)
        ADD_BOOLPROP_FLAG(TriggerClose, LTFALSE, PF_GROUP4)
        ADD_BOOLPROP_FLAG(RemainsOpen, LTFALSE, PF_GROUP4)
        ADD_BOOLPROP_FLAG(ForceMove, LTFALSE, PF_GROUP4)
	    ADD_BOOLPROP_FLAG_HELP(AlienTearObject, LTTRUE, PF_GROUP4, "Set this to true if you want the door to act as an Alien tearable object.  ActivateTrigger must be set to FALSE above.")
	ADD_VISIBLE_FLAG(1, 0)
	ADD_SOLID_FLAG(1, 0)
    ADD_BOOLPROP(BlockLight, LTFALSE) // Used by pre-processor
    ADD_BOOLPROP(BoxPhysics, LTTRUE)
	...
	ADD_LONGINTPROP(LightFrames, 1)
	ADD_STRINGPROP_FLAG(DoorLink, "", PF_OBJECTLINK)
	ADD_STRINGPROP_FLAG_HELP(TearSound, "TFX_BasicTear", PF_STATICLIST, "This sound will be played when the door takes tear damage.")
END_CLASS_DEFAULT_FLAGS_PLUGIN(AlienTearDoor, Door, NULL, NULL, 0, CDoorPlugin)
	...
	HPREOBJECT hObject)
{
	...
	while(pInterface->Parse(&cParse) == LT_OK)
	{
		...
		for(iStep=0; iStep < (uLightFrames+1); iStep++)
		{
			...
			pLightInfo->m_BrightScale = theProps.m_fBrightScale;
    ...
    const uint32 cMaxStringLength)
{
	...
	if((_stricmp("TearSound", szPropName) == 0))
	{
		m_SoundButeMgrPlugin.PreHook_EditStringList(szRezPath, szPropName, 
			aszStrings, pcStrings, cMaxStrings, cMaxStringLength, "BaseTearFXSound");
...
Door::Door() : GameBase(OT_WORLDMODEL)
{
    ...
    m_hstrTearSound			= LTNULL;
    m_hstrAttachments       = LTNULL;
    m_hstrOpenCmd           = LTNULL;
	m_hstrOpeningCmd		= LTNULL;
    m_hstrCloseCmd          = LTNULL;
	m_hstrClosingCmd		= LTNULL;
...
Door::~Door()
{
	...
	if (m_hstrTearSound)
	{
        g_pLTServer->FreeString(m_hstrTearSound);
        m_hstrTearSound = LTNULL;
	}
	if (m_hstrCloseSound)
	{
	...
	if (m_hstrOpeningCmd)
	{
        g_pLTServer->FreeString(m_hstrOpeningCmd);
		m_hstrOpeningCmd = NULL;
	}
	if (m_hstrCloseCmd)
	{
	...
	if (m_hstrClosingCmd)
	{
        g_pLTServer->FreeString(m_hstrClosingCmd);
		m_hstrClosingCmd = NULL;
	}
	if (m_hstrDoorLink)
	{
	...
	DoorList::iterator iter_to_me = std::find(m_sDoorList.begin(), m_sDoorList.end(), this);
	if( iter_to_me != m_sDoorList.end() )
		m_sDoorList.erase(iter_to_me);
}
...
LTBOOL Door::ReadProp(ObjectCreateStruct *)
{
    ...
    g_pLTServer->GetPropBool("AlienTearObject", &bFlag);
	m_dwStateFlags |= bFlag ? DF_TEAROBJECT : 0;
    ...
    if (g_pLTServer->GetPropGeneric("TearSound", &genProp) == LT_OK)
	{
		if (genProp.m_String[0])
		{
            m_hstrTearSound = g_pLTServer->CreateString(genProp.m_String);
		}
	}
    ...
    if (g_pLTServer->GetPropGeneric("OpeningCommand", &genProp) == LT_OK)
	{
		if (genProp.m_String[0])
		{
            m_hstrOpeningCmd = g_pLTServer->CreateString(genProp.m_String);
		}
	}
    ...
    if (g_pLTServer->GetPropGeneric("ClosingCommand", &genProp) == LT_OK)
	{
		if (genProp.m_String[0])
		{
            m_hstrClosingCmd = g_pLTServer->CreateString(genProp.m_String);
		}
	}
...
//		{
			if (m_hDoorLink)
			{
                Door* pDoorLink = dynamic_cast<Door*>(g_pLTServer->HandleToObject(m_hDoorLink));
				if (pDoorLink)
				{
...
LTBOOL Door::InitialUpdate(int nInfo)
{
	...
	{
		dwUsrFlgs = g_pLTServer->GetObjectUserFlags(m_hObject);
	...
	else
	{
		...
		{
			//This change will be lost if not done before
			//GetUserFlags is called.
			SetActivateType(m_hObject, AT_NORMAL);
		}
		else if (m_dwStateFlags & DF_TEAROBJECT)
		{
			//This change will be lost if not done before
			//GetUserFlags is called.
			SetActivateType(m_hObject, AT_TEAR);
		}
	...
	// Make sure the door state is initialized to closed...
...
//	ObjectList *pList;
	...
	if(m_hShadowLightsString)
	{
        ...
        while (g_pLTServer->Common()->Parse(&cParse) == LT_OK)
		{
            ...
            if (IsKeyframerLight(g_pLTServer, hLight) &&
                (pKeyframerLight = dynamic_cast<KeyframerLight*>(g_pLTServer->HandleToObject(hLight))))
			{
	...
	{
		g_pLTServer->TeleportObject(m_hObject, &m_vOpenPos);
		SetOpen(LTTRUE);

		// Make sure the portal is set properly...
		if (m_hstrPortalName)
		{
...
void Door::SetOpening()
{
	...
	if (m_hstrOpeningCmd)
	{
        char* pCmd = g_pLTServer->GetStringData(m_hstrOpeningCmd);
...
void Door::SetClosing()
{
	...
	if (m_hstrClosingCmd)
	{
        char* pCmd = g_pLTServer->GetStringData(m_hstrClosingCmd);
...
void Door::TriggerMsg(HOBJECT hSender, HSTRING hMsg)
{
    ...
    g_pLTServer->SetDeactivationTime(m_hObject, DOOR_DEFAULT_DEACTIVATION_TIME);
	...
	while (pCommon->Parse(&parse) == LT_OK)
	{
		if (parse.m_nArgs > 0 && parse.m_Args[0])
		{
			...
			{
                char* pObj = parse.m_nArgs > 1 ? parse.m_Args[1] : LTNULL;
				HandleAttach(pObj);
			}
			else if (_stricmp(parse.m_Args[0], g_szAttachList) == 0)
			{
                char* pObjects = parse.m_nArgs > 1 ? parse.m_Args[1] : LTNULL;
			...
			else if (_stricmp(parse.m_Args[0], g_szDetachList) == 0)
			{
				if (m_pAttachmentList)
				{
					ObjectLink* pLink = m_pAttachmentList->m_pFirstLink;
...
uint32 Door::ObjectMessageFn(HOBJECT hSender, uint32 messageID, HMESSAGEREAD hRead)
{
	switch (messageID)
	{
		...
		case MID_DAMAGE:
		{
			if(m_dwStateFlags & DF_TEAROBJECT)
			{
				DamageStruct damage;
				damage.InitFromMessage(hRead);

				if(damage.eType == DT_TEAR)
				{
					// Play the sound...
					if(m_hstrTearSound)
					{
					    char *pSoundBute = g_pLTServer->GetStringData(m_hstrTearSound);
						if(pSoundBute)
						{
							LTVector vPos;
							g_pLTServer->GetObjectPos(m_hObject, &vPos);
							g_pServerSoundMgr->PlaySoundFromPos(vPos, pSoundBute);
						}
					}
				}
			}
...
void Door::Save(HMESSAGEWRITE hWrite, uint32 dwSaveFlags)
{
    ...
    g_pLTServer->WriteToMessageFloat(hWrite, m_fMoveStartTime - g_pLTServer->GetTime());
    g_pLTServer->WriteToMessageFloat(hWrite, m_fMoveDelay);
    g_pLTServer->WriteToMessageFloat(hWrite, m_fDoorStopTime - g_pLTServer->GetTime());
    ...
    g_pLTServer->WriteToMessageHString(hWrite, m_hstrTearSound);
    g_pLTServer->WriteToMessageHString(hWrite, m_hstrAttachments);
    g_pLTServer->WriteToMessageHString(hWrite, m_hstrOpenCmd);
	g_pLTServer->WriteToMessageHString(hWrite, m_hstrOpeningCmd);
    g_pLTServer->WriteToMessageHString(hWrite, m_hstrCloseCmd);
	g_pLTServer->WriteToMessageHString(hWrite, m_hstrClosingCmd);
...
void Door::Load(HMESSAGEREAD hRead, uint32 dwLoadFlags)
{
    ...
    m_fMoveStartTime        = g_pLTServer->ReadFromMessageFloat(hRead) + g_pLTServer->GetTime();
    m_fMoveDelay            = g_pLTServer->ReadFromMessageFloat(hRead);
    m_fDoorStopTime         = g_pLTServer->ReadFromMessageFloat(hRead) + g_pLTServer->GetTime();
    ...
    m_hstrTearSound		    = g_pLTServer->ReadFromMessageHString(hRead);
    m_hstrAttachments       = g_pLTServer->ReadFromMessageHString(hRead);
    m_hstrOpenCmd           = g_pLTServer->ReadFromMessageHString(hRead);
	m_hstrOpeningCmd        = g_pLTServer->ReadFromMessageHString(hRead);
    m_hstrCloseCmd          = g_pLTServer->ReadFromMessageHString(hRead);
	m_hstrClosingCmd		= g_pLTServer->ReadFromMessageHString(hRead);
