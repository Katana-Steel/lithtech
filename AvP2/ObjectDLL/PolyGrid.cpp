...
BEGIN_CLASS(PolyGrid)
    ...
    ADD_REALPROP(ZPan, 10.0f)   
	ADD_REALPROP(Frequency, 50.0f)
	ADD_REALPROP(Alpha, 0.7f)
	ADD_BOOLPROP(Additive, DFALSE)
	ADD_BOOLPROP(Multiply, DFALSE)
...
void PolyGrid::	Setup(DVector* pvDims, DVector* pvColor1, DVector* pvColor2,
					  ...
					  DFLOAT fAlpha, DDWORD dwNumPolies, DBOOL bAdditive,
					  DBOOL bMultiply)
{
	...
	pServerDE->WriteToMessageFloat(hMessage, fFrequency);
			...
			if (fData != INITIALUPDATE_SAVEGAME)
			{
				...
				if (m_bSetup) 
				{
					Setup(&m_vDims, &m_vColor1, &m_vColor2, m_hstrSurfaceSprite, 
						  ...
						  m_fXScaleDuration, m_fYScaleDuration, m_fXPan, m_fYPan, m_fFrequency,
...
void PolyGrid::ReadProp(ObjectCreateStruct * pStruct)
{
	...
	pServerDE->GetPropReal("ZPan", &m_fYPan);
	pServerDE->GetPropReal("Frequency", &m_fFrequency);
