...
Group::~Group()
{
	for (int i=0; i < MAX_GROUP_TARGETS; i++)
	{
		...
		{
			g_pServerDE->FreeString(m_hstrObjectNames[i]);
		}
	}
}
...
DWORD Group::EngineMessageFn(DWORD messageID, void *pData, DFLOAT fData)
{
	switch(messageID)
	{
...
void Group::PreCreate(ObjectCreateStruct *pStruct)
{
	...
	for (int i=0; i < MAX_GROUP_TARGETS; i++)
	{
		...
		if (g_pServerDE->GetPropGeneric(buf, &gProp) == LT_OK)
		{
			if (gProp.m_String[0])
			{
				m_hstrObjectNames[i] = g_pServerDE->CreateString(gProp.m_String);
			}
		}
	}
}
...
void Group::HandleTrigger(HOBJECT hSender, char *pMsg)
{
	...
	for (int i=0; i < MAX_GROUP_TARGETS; i++)
	{
		if (m_hstrObjectNames[i])
		{
			pName = g_pServerDE->GetStringData(m_hstrObjectNames[i]);
			if (pName && pName[0])
			{
				SendTriggerMsgToObjects(this, pName, pMsg);
			}
		}
	}
}
