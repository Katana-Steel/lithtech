...
BEGIN_CLASS(ScaleSprite)
	...
	ADD_STRINGPROP_FLAG_HELP(Filename, "", PF_FILENAME, "If you intend to use the Lenseflare props, you must have a valid /n sprite in this field.  Additionally, if you list a sprite in the SpriteFile field /n of the Lenseflare props, that field will override this one.")
	...
	PROP_DEFINEGROUP(AdditionalFlags, PF_GROUP2)
        ADD_BOOLPROP_FLAG_HELP(FlushWithWorld, LTFALSE, PF_GROUP2,		"Same as a rotateable sprite except it aligns itself to whatever is behind it.")
        ADD_BOOLPROP_FLAG_HELP(Rotatable, LTFALSE, PF_GROUP2,			"Allows the sprite to face a particular forward direction, instead of always facing at the camera.")
        ADD_BOOLPROP_FLAG_HELP(Glow, LTTRUE, PF_GROUP2,					"Sets the glow sprite flag.")
        ADD_BOOLPROP_FLAG_HELP(ZBias, LTTRUE, PF_GROUP2,				"Renders this sprite in front of everything.")
        ADD_BOOLPROP_FLAG_HELP(Additive, LTTRUE, PF_GROUP2,				"Use additive blending.")
        ADD_BOOLPROP_FLAG_HELP(Multiply, LTFALSE, PF_GROUP2,			"Use multiply blending.")
        ADD_BOOLPROP_FLAG_HELP(UseLighting, LTFALSE, PF_GROUP2,			"Allow the color of this sprite to be modified by surrounding lights.")
		ADD_BOOLPROP_FLAG_HELP(FogDisable, LTTRUE, PF_GROUP2,			"Disables fog effects on the sprite.")
...
uint32 ScaleSprite::ObjectMessageFn(HOBJECT hSender, uint32 messageID, HMESSAGEREAD hRead)
{
	switch(messageID)
	{
 		case MID_TRIGGER:
		{
            ...
            char *pszMessage = g_pLTServer->GetStringData(hMsg);
			...
			else if (stricmp(pszMessage, "ON") == 0)
			{
				if (m_hObject)
				{
...
void ScaleSprite::ReadProp(ObjectCreateStruct *pStruct)
{
    ...
    if (g_pLTServer->GetPropGeneric("UseLighting", &genProp) == LT_OK)
	{
		if (!genProp.m_Bool)
		{
			pStruct->m_Flags  |= FLAG_NOLIGHT;
		}
	}
