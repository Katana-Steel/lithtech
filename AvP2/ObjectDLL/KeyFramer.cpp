...
void KeyframerPreprocessorCB(HPREOBJECT hObject, ILTPreLight *pInterface)
{
	...
	if(pInterface->GetPropGeneric(hObject, "ShadowLights", &gProp) == LT_OK)
	{
		...
		while(pInterface->Parse(&cParse) == LT_OK)
		{
			...
			pLightInfo->m_BrightScale = props.m_fBrightScale;
...
KeyFramer::KeyFramer() : GameBase(OT_NORMAL)
{
	...
	m_bInterpolateRotation= LTFALSE;
	m_bLooping			= LTFALSE;
	m_bActive			= LTFALSE;
	m_bPaused			= LTFALSE;
	m_bFinished			= LTFALSE;
...
void KeyFramer::GoActive(LTBOOL bReset)
{
    ...
    g_pLTServer->SetNextUpdate(m_hObject, KF_UPDATE_DELTA);
	...
	while (pCommon->Parse(&parse) == LT_OK)
	{
		if (parse.m_nArgs > 0 && parse.m_Args[0])
		{
			...
			for (i = 0; i < numObjects; i++)
			{
				...
				// Set the flag so the motion detector will go off...
				uint32 dwUsrFlags = g_pServerDE->GetObjectUserFlags(hObject);
				g_pServerDE->SetObjectUserFlags(hObject, dwUsrFlags | USRFLG_KEYFRAMED_MOVEMENT);
			}
		}
	}
...
void KeyFramer::TriggerMsg(HOBJECT hSender, char* pMsg)
{
	if (!pMsg) return;
...
LTBOOL KeyFramer::ReadProps()
{
    ...
    if (g_pLTServer->GetPropGeneric("InterpolateRotation", &genProp) == LT_OK)
	{
		m_bInterpolateRotation = genProp.m_Bool;
	}
...
void KeyFramer::UpdateObjects(LTBOOL bTeleport)
{
...
void KeyFramer::Save(HMESSAGEWRITE hWrite, uint32 dwSaveFlags)
{
    ...
    g_pLTServer->WriteToMessageFloat(hWrite, m_fEarliestGoActiveTime - g_pLTServer->GetTime());
    ...
    g_pLTServer->WriteToMessageByte(hWrite, m_bInterpolateRotation);
...
void KeyFramer::Load(HMESSAGEREAD hRead, uint32 dwLoadFlags)
{
    ...
    m_fEarliestGoActiveTime = g_pLTServer->ReadFromMessageFloat(hRead) + g_pLTServer->GetTime();
    ...
    m_bInterpolateRotation= (LTBOOL) g_pLTServer->ReadFromMessageByte(hRead);
