...
void CDEditLightMapMgr::DirtyBrushSimple(CEditBrush* pBrush)
{
	...
	if(!((CPolyLightMap *)pBrush)->IsInDirtyList())
	{
		m_Generator.DirtyVertexHolder((ILitVertexHolder *)pBrush);
		((CPolyLightMap *)pBrush)->AddedToDirtyList();
	}
...
void CDEditLightMapMgr::RemoveBrush(CEditBrush* pBrush)
{
	...
	m_Generator.RemoveVertexHolder((ILitVertexHolder *)pBrush);
	...
	for(uint32 nCurrPoly = 0; nCurrPoly < pBrush->m_Polies.GetSize(); nCurrPoly++)
	{
		...
		if(bInPoly)
		{
			...
			else
			{
				...
				m_Generator.DirtyLightMapHolder((ILightMapHolder *)pPoly, true);
			}
